...
int32_t tmp1 =  (int32_t)0;

uint64_t tmp2;

uint64_t* tmp3 = make_array<uint64_t>( (int32_t)256);
for (uint32_t i0 =  (int32_t)0; i0 <  (int32_t)256; i0++){
tmp2 = funcSSCons( (int64_t)0);
for (uint32_t i1 =  (int32_t)0; i1 <  (int32_t)50; i1++){
tmp2 = SecretAdd(tmp2, Arr2DIdxRowM(tmp0, (int32_t)50, (int32_t)256,i1,i0));
}
Arr1DIdxRowM(tmp3, (int32_t)256,i0) = tmp2;
}
ClearMemSecret2( (int32_t)50,  (int32_t)256, tmp0);
EndComputation();
for (uint32_t i0 =  (uint32_t)0; i0 <  (int32_t)256; i0++){
cout << (funcReconstruct2PCCons(Arr1DIdxRowM(tmp3, (int32_t)256,i0), 2)) << endl;
}
}
