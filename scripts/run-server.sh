...
then
  ...
  if ! contains "cheetah SCI_HE" $1; then
    echo -e "Usage: run-server.sh ${RED}[cheetah|SCI_HE]${NC} [sqnet|resnet50|densenet121|Lenet|vgg11]"
	exit 1
  fi

  if ! contains "sqnet resnet50 densenet121 Lenet vgg11" $2; then
    echo -e "Usage: run-server.sh [cheetah|SCI_HE] ${RED}[sqnet|resnet50|densenet121|Lenet|vgg11]${NC}"
	exit 1
  fi
  ...
  echo -e "build/bin/$2-$1 r=1 k=${FXP_SCALE} ell=${SS_BITLEN} nt=${NUM_THREADS} p=${SERVER_PORT}"
